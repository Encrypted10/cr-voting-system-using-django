...
from .forms import UserRegisterForm, AdminUserAddForm, NomineeForm, FeedbackForm
from .models import Nominee, Vote, CustomUser,Feedback
...
def cast_vote(request):
    ...
    if request.method == 'POST':
        ...
        if Vote.objects.filter(user=request.user, election_id=request.user.election_id).exists():
            messages.error(request, "You have already voted in this election.")
            return redirect('cast_vote')  # Redirect back to the voting page
        
        # Create a new Vote object
        Vote.objects.create(user=request.user, nominee=nominee, election_id=request.user.election_id)
        
        # Redirect to success page after successful vote
        return redirect('success')  # Assuming 'vote_success' is the name of your success URL
